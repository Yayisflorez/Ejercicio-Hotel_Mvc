...
function editarReserva(id) {
  ...
  if (pagoTexto.includes('nequi')) { document.getElementById('edit-pago-nequi').checked = true; pagoValue = 'nequi'; }
  else if (pagoTexto.includes('daviplata')) { document.getElementById('edit-pago-daviplata').checked = true; pagoValue = 'daviplata'; }
  else { document.getElementById('edit-pago-bancolombia').checked = true; pagoValue = 'bancolombia'; }
...
async function cambiarCategoriaEdit(preselectId = null) {
  ...
  try {
    const response = await fetch(`model/Habitacion.php?action=getHabitacionesByCategoria&categoria=${encodeURIComponent(categoria)}`);
...
function cambiarHabitacionEdit() {
  ...
  document.getElementById('edit-img').src = img || 'https://images.unsplash.com/photo-1631049307264-da0ec9d70304?auto=format&fit=crop&w=900&q=80';
  ...
  const currentPersonas = parseInt(document.getElementById('edit-personas-input').value);
  if (currentPersonas > maxPersonas) {
      ...
      document.getElementById('edit-personas-plural').style.display = maxPersonas > 1 ? 'inline' : 'none';
      showToast('⚠️', 'Máximo ajustado', `La habitación seleccionada permite máximo ${maxPersonas} personas.`);
  }
...
function calcularTotalEdit() {
  if (editFechaInicio && editFechaFin && editPrecioActual > 0) {
      ...
      if(resNoches) resNoches.textContent = noches > 0 ? noches : '—';
      if(resTotal) resTotal.textContent = (noches > 0) ? '$' + (noches * editPrecioActual).toLocaleString() : '—';
  } else {
      const resNoches = document.getElementById('res-edit-noches');
      const resTotal = document.getElementById('res-edit-total');
...
function validarCambiosEdit() {
  const currentState = {
    ...
    pago: document.querySelector('input[name="pago_edit"]:checked')?.value || ''
  };

  const btn = document.getElementById('btn-guardar-edit');
  if(!btn) return;
  
  if (!currentState.categoria || !currentState.idHabitacion || !currentState.fechaInicio || !currentState.fechaFin) {
    btn.disabled = true;
    return;
  }
...
document.getElementById('formEditar').addEventListener('submit', function(e) {
  ...
  // Validar habitación
  if (!idHab || !precio) {
    showToast('⚠️', 'Habitación incompleta', 'Selecciona una habitación válida.');
    return;
  }
...
function descargarReserva(id) {
  window.location.href = `index.php?action=descargarPdfReserva&id=${id}`;
  showToast('📄', 'Generando PDF', `Descargando reserva ${id}...`);
}
