...
class RegisterController
{
    ...
    {  
        ...
        elseif (!preg_match('/^[A-Za-zÁÉÍÓÚáéíóúÑñ\s]+$/u', $Nombre)) $errors['nombre'] = "El nombre solo debe contener letras";
        elseif (mb_strlen($Nombre) < 3) $errors['nombre'] = "El nombre debe tener al menos 3 caracteres";

        if (empty($Apellido)) $errors['apellido'] = "El apellido es obligatorio";
        elseif (!preg_match('/^[A-Za-zÁÉÍÓÚáéíóúÑñ\s]+$/u', $Apellido)) $errors['apellido'] = "El apellido solo debe contener letras";

        if (empty($TipoDocId)) $errors['tipo_documento_id'] = "El tipo de documento es obligatorio";

        if (empty($Documento)) $errors['documento'] = "El número de documento es obligatorio";
        elseif (!preg_match('/^\d+$/', str_replace(' ', '', $Documento))) $errors['documento'] = "El número de documento solo debe contener números";

        if (empty($Telefono)) $errors['telefono'] = "El teléfono es obligatorio";
        elseif (!preg_match('/^\d+$/', str_replace(' ', '', $Telefono))) $errors['telefono'] = "El teléfono solo debe contener números";
        ...
        if (empty($Contraseña1)) $errors['password'] = "La contraseña es obligatoria";
        elseif (mb_strlen($Contraseña1) < 8) $errors['password'] = "Mínimo 8 caracteres";

        if (empty($Contraseña2)) $errors['confirm_password'] = "Confirma la contraseña";
        elseif ($Contraseña1 !== $Contraseña2) $errors['confirm_password'] = "No coinciden";
        ...
        // Validar email y documento únicos
        $usuarioModel = new Usuario();
        if ($usuarioModel->emailExiste($Email)) $errors['email'] = "El email ya está registrado";
        if ($usuarioModel->documentoExiste($Documento)) $errors['documento'] = "El número de documento ya está registrado";
